...
fn run_code(code: &str, memory: &mut Memory, effects: &HashMap<char, CmdEffect>) -> io::Result<()> {
...
fn repl(memory: &mut Memory, effects: &HashMap<char, CmdEffect>) -> io::Result<()> {
    let stdin = io::stdin();
    let mut buffer = String::new();

    println!("Hecksagon REPL (type 'exit' to quit)");
    loop {
        print!("> ");
        io::stdout().flush()?;
        buffer.clear();
...
fn main() -> io::Result<()> {
    ...
    if args.len() < 2 || args[1] == "--repl" {
        ...
        run_code(&code, &mut memory, &effects)?;
    }
